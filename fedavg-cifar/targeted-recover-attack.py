...
#    aux=args.aux
    ...
    encoder=Encoder()
    encoder.load_state_dict(torch.load("models/encoder_model_targeted.pkl"))
    encoder.to(device)
    decoder=Decoder()
    decoder.load_state_dict(torch.load("models/decoder_model_targeted.pkl"))
    decoder.to(device)
    ...
    bins=torch.load("data/latent_small_bins_targeted.pt")
