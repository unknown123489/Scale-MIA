...
#    test_data = torchvision.datasets.FashionMNIST(root='./test', train=True, transform=ToTensor(), download=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=False)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    
    encoder=Encoder()
    encoder.load_state_dict(torch.load("models/encoder_model_"+str(aux)+".pkl"))
    encoder.to(device)
    decoder=Decoder()
    decoder.load_state_dict(torch.load("models/decoder_model_"+str(aux)+".pkl"))
    decoder.to(device)
    ...
    bins=torch.load("data/latent_small_bins_"+str(aux)+".pt")
